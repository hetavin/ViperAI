...
// esc() does not touch quotes, so it is unsafe inside an HTML attribute.
const escAttr = t => String(t == null ? '' : t)
    .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;').replace(/'/g, '&#39;');
// Model output is influenced by user input and by web-search snippets, so a
// link target is untrusted: allow only navigable schemes, never javascript:.
const safeUrl = u => {
    try {
...
function rt(t) {
    ...
    h = h.replace(/(?<!href="|src="|=")https?:\/\/[^\s<>"']+/g, url => {
        // This runs after escaping, so query separators arrive as &amp;.
        ...
        return `<a href="${escAttr(href)}" target="_blank" rel="noopener noreferrer" class="md-link" data-url="${escAttr(href)}">${esc(label)} <i class="fas fa-external-link-alt" style="font-size:9px;opacity:.6"></i></a>`;
    });
    // horizontal rule
    ...
    fetch('/chat', fetchOpts)
        .then(r => {
            if (!r.ok) return r.text().then(t => {
                // Surface the server's own message rather than a raw JSON dump.
...
function renderFilePreview() {
    ...
    // Object URLs pin the file in memory until revoked; drop the previous batch
    ...
    el.innerHTML = attachedFiles.map((f, i) => {
        ...
        let preview = `<i class="fas fa-file" style="font-size:16px;color:var(--accent)"></i>`;
        if (isImg) {
            const u = URL.createObjectURL(f);
            _previewUrls.push(u);
            preview = `<img src="${escAttr(u)}">`;
        }
        return `<div class="ia-file-chip">${preview}<span title="${escAttr(f.name)}">${esc(f.name)}</span><button onclick="removeFile(${i})"><i class="fas fa-xmark"></i></button></div>`;
    }).join('');
}
...
(function () {
    ...
    document.addEventListener('touchstart', e => {
        ...
        const sbVisible = innerWidth > 768 || sb.classList.contains('open');
        if (window.scrollY === 0 && sbVisible) { startY = e.touches[0].clientY; pulling = true; }
    }, { passive: true });
    ...
    document.addEventListener('touchend', e => {
        ...
        const sb = document.getElementById('sb');
        const sbVisible = innerWidth > 768 || sb.classList.contains('open');
        if (dist >= threshold && sbVisible) window.location.reload();
    });
}());
